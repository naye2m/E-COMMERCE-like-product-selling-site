...
NUM_OF_COMMENTS = 5 # default num of comments to display
...
class SellsPageUser(User):
    ...
    contracts = models.OneToOneField(
        "Contracts", on_delete=models.CASCADE, blank=True, null=True
    )
    ...
    def get_page(self, page_no: int = 1) -> dict:
        ...
        return {
            ...
            "products": [
                {
                    **filter_dict(post.serialize(), propsNeeded),
                    ...
                    "likesCU": post.likes.filter(user=self).exists(),
                }
            ...
            "previousPage": page.previous_page_number()
...
class Contracts(models.Model):
    whatsapp_phone = models.CharField(
        _("Whatsapp Phone Number"), max_length=20, blank=True, null=True
    )
    whatsapp_link = models.URLField(
        _("WhatsApp Link"), max_length=200, blank=True, null=True
    )
    imo_phone = models.CharField(
        _("IMO Phone Number"), max_length=20, blank=True, null=True
    )
    phone = models.CharField(
        _("Personal Phone Number"), max_length=20, blank=True, null=True
    )
    bkash_phone = models.CharField(
        _("Bkash Phone Number"), max_length=20, blank=True, null=True
    )
    nagad_phone = models.CharField(
        _("Nagad Phone Number"), max_length=20, blank=True, null=True
    )
    facebook_username = models.CharField(
        _("Facebook Username"), max_length=50, blank=True, null=True
    )
    messenger_username = models.CharField(
        _("Messenger Username"), max_length=50, blank=True, null=True
    )
    facebook_link = models.URLField(
        _("Facebook Link"), max_length=200, blank=True, null=True
    )
    messenger_link = models.URLField(
        _("Messenger Link"), max_length=200, blank=True, null=True
    )


class Promo(models.Model):
    holder = models.ForeignKey(
        SellsPageUser, related_name="my_promos", on_delete=models.CASCADE
    )
...
class Product(models.Model):
    ...
    listed_by = models.ForeignKey(
        SellsPageUser, related_name="listed_products", on_delete=models.CASCADE
    )
    ...
    product_type = models.CharField(
        max_length=20, choices=choiceListGen(productTypes), default=productTypes[0]
    )
    # models.Choice(fsd=5)
    ...
    def get_absolute_url(self):
        return reverse("sellspage:product_detail", args=[str(self.id)])

    def __str__(self):
        return f"{self.title} by {self.listed_by.username}"

    def comments_default(self) ->dict:
        return self.get_comments(NUM_OF_COMMENTS)

    def get_comments(self, num=None) -> dict:
        comms = self.comments.order_by("-id").prefetch_related("user","product")
        try:
            if num is not None:
                comms = comms[:int(num)]
        except:
        ...
        comms = comms.values("comment","user__username","created_at","product__id")
        
        for comm in comms:
            comm["timestamp"] = timestampFormatter(comm["created_at"])
    ...
    def serialize(self) -> dict:
        ...
        return {
            ...
            # "likes":"todo",
            "likes_c": self.likes.count(),
            "comments":self.comments_default(),
        }
...
class KeyFeatureOfProduct(models.Model):
    ...
    product = models.ForeignKey(
        Product, related_name="key_features", on_delete=models.CASCADE
    )


class DescriptionPhoto(models.Model):
    product = models.ForeignKey(
        Product, related_name="description_photos", on_delete=models.CASCADE
    )
    photo = models.ForeignKey(
        PhotoModel, verbose_name=_("Description Photo"), on_delete=models.CASCADE
    )
...
class Order(models.Model):
    ...
    user = models.ForeignKey(
        SellsPageUser, related_name="orders", on_delete=models.CASCADE
    )
...
class Comment(models.Model):
    ...
    user = models.ForeignKey(
        SellsPageUser, related_name="comments", on_delete=models.CASCADE
    )
...
class Wishlist(models.Model):
    user = models.ForeignKey(
        SellsPageUser, related_name="wishlists", on_delete=models.CASCADE
    )
    product = models.ForeignKey(
